	...
	*/
    ...
    private int generate_runs(int max_elems, AttrType sortFldType, int sortFldLen)
        ...
        while (true) {
            ...
            switch(order_type) {
                case 1:
                    comp_res = MapUtils.compareMapWithMap_one(cur_node.map, lastElem);
                    break;
                case 2:
                    comp_res = MapUtils.compareMapWithMap_two(cur_node.map, lastElem);
                    break;
                case 3:
                    comp_res = MapUtils.compareMapWithMap_three(cur_node.map, lastElem);
                    break;
                case 4:
                    comp_res = MapUtils.compareMapWithMap_four(cur_node.map, lastElem);
                    break;
                case 5:
                    comp_res = MapUtils.compareMapWithMap_five(cur_node.map, lastElem);
                    break;
                case 6:
                    comp_res = MapUtils.compareMapWithMap_six(cur_node.map, lastElem);
                    break;
                case 7:
                    comp_res = MapUtils.CompareMapWithMapValues(cur_node.map, lastElem);
                    break;
                case 8:
                	comp_res = MapUtils.CompareMapWithMapRows(cur_node.map, lastElem);
                    break;
                case 9:
                	comp_res = MapUtils.compareMapWithMap_seven(cur_node.map, lastElem);
                    break;
            }
